...
export function LightLinesPanel() {
  ...
  return (
    <div className="space-y-6">
      <section className="ls-card p-5">
        <h2 className="ls-heading mb-1 text-lg">Lichtlijnen</h2>
        <p className="mb-4 text-xs text-[var(--ls-navy-muted)]">
        ...
        >
          ...
          <p className="mt-1 text-sm text-[var(--ls-navy-muted)]">{fillAdvice}</p>
        </div>
        ...
        <div className="space-y-4">
          {lines.map((line) => (
            ...
            >
              <h3 className="text-sm font-semibold text-[var(--ls-navy)]">
      ...
      <section className="ls-card p-5">
        <h2 className="ls-heading mb-4 text-lg">Installatie &amp; dimmen</h2>
        ...
        <div className="mt-4 rounded-lg border border-slate-100 bg-slate-50 p-4">
          <h3 className="text-sm font-semibold text-[var(--ls-navy)]">Vermogen</h3>
        ...
        <div className="ls-accent-panel mt-4 p-4">
          <p className="text-sm font-medium text-[var(--ls-navy)]">{summary.advice}</p>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between">
      <dt className="text-[var(--ls-navy-muted)]">{label}</dt>
      <dd className="font-medium text-[var(--ls-navy)]">{value}</dd>
    </div>
  );
}
...
function RowBlock({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between border-b border-slate-100 pb-2">
      <dt className="text-[var(--ls-navy-muted)]">{label}</dt>
      <dd className="font-semibold text-[var(--ls-navy)]">{value}</dd>
    </div>
  );
}
